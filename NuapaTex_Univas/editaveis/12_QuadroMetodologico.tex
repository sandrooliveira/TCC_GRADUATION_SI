...
\par O método \texttt{calculateValue()}, após agrupar os cromossomos por atividade, 
 ...
 atividadeCromossomos conforme mostra o Código ~\ref{list:metodoCalculateValue}.
...
~\ref{fig:montagem_node}, será criada. O Código ~\ref{list:classeNode} mostra a construção de um objeto \texttt{Node}.
...
\par Como se pode ver no método \texttt{calculateValue()} no Código ~\ref{list:metodoCalculateValue}, após criar a estrutura de nós, é chamado o método \texttt{getTempoTotal()} do objeto
...
\par A função de avaliação é chamada dentro do método \texttt{classify}, dentro da classe GAController, tal método, 
...
\par Após chamar a função de avaliação para todos os indivíduos, através do método calculateValue(), o método
...
\begin{lstlisting} [style=custom_Java,caption={[Documento XTML da tela de distribuição]{Documento XTML da tela de distribuição. \textbf{Fonte:} Elaborado pelos autores.}}, label=list:doc_xhtml] 
